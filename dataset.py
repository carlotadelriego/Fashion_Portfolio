...
def preprocess_image(image_path):
    ...
    img = img / 255.0
    return img  # Eliminamos np.expand_dims para evitar la dimensión extra
...
output_layer_style = layers.Dense(5, activation='softmax')(x)  # Ajusta el número de clases
...
# Convertimos los datos a numpy arrays con la forma correcta
X = np.array([x[0] for x in processed_data], dtype=np.float32)  # Forma (num_imagenes, 224, 224, 3)
y = np.array([x[1] for x in processed_data])

# Codificar etiquetas
label_encoder = LabelEncoder()
y = label_encoder.fit_transform(y)
y = to_categorical(y)  # Convertimos las etiquetas en one-hot encoding
...
def get_similar_items(uploaded_file, X_features):
    if uploaded_file is not None:
        ...
        input_img = np.expand_dims(input_img, axis=0)  # Agregamos dimensión de batch

        # Extraer características para similitud
        ...
        style_label = np.argmax(style_prediction)  # Índice del estilo predicho
...
if uploaded_file:
    ...
    if style_label is not None:
        style_name = label_encoder.inverse_transform([style_label])[0]  # Obtener nombre del estilo
